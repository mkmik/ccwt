...
var cli struct {
	...
	Ws                WsCmd                `cmd:"" name:"ws" help:"The tui for the first tab of a herdr workspace: asks for a seed prompt and starts an agent on it in a tab of its own (task_command run with it in the workspace's worktree), and lists the workspace's tabs — arrows select one, space goes to it, n asks for another prompt."`
	...
	Mr                MrCmd                `cmd:"" name:"mr" aliases:"pr" help:"Say where a review stands and what its pipeline did: \"ccwt mr <merge request or pull request url>\" for that one, or \"ccwt mr PROJ-1234\" (a Jira key, or its browse url) for a row per review that names the ticket. Asks glab or gh, whichever forge origin is on."`
