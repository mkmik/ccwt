...
* * dreamy-foraging-hickey  worktree-dreamy-…-hickey  2h   yes    ✳ Goal was a widget on the dashboard; it's built and…
 ...
 ☰  q:quit  p:pull  g:git  /:search  l:log  x:new  space:open  n:queue  m:vcs  d:details  r:remove │ main ↑1
```
...
```
 ☰  q:quit  p:pull  /:search │ upgraded to v0.64.0 — restart ccwt
```
...
creates a worktree and opens it without leaving the list, and `c` does that and starts the
agent cli in it — `claude`, or whatever `task_command` says. These keys only exist when the tui
...
```
  TAB                     AGENT    DIR                     TITLE
* 1                                ccwt                    zsh
  dreamy-foraging-hickey  working  dreamy-foraging-hickey  Wiring the widget to real numbers
  calm-baking-otter       idle     calm-baking-otter       Waiting on your review

 ☰  q:quit  /:search  n:agent  space:go  g:git │ started calm-baking-otter
```

`n` opens the same box the [queue](#queued-prompts) types into, and what goes in it is a seed
prompt: `↵` makes a fresh worktree, opens a tab of the workspace in it, named after it, and
runs `task_command` there with the prompt — what `space` on a `<new>` row does, in a tab of
the workspace you're in rather than a workspace of its own. The tab opens behind the one
you're typing in, so the table shows it and you go when you're ready. A workspace with no
...
selected tab, and `/`, the mouse and the `☰` are the tui's own. This is the first stage of a
longer plan — what each tab is about, and which tickets and reviews it touches — and
`TODO.md` has the rest of it.
...
| `ccwt config view` / `ccwt config edit` | Print the config file, or open it in `$EDITOR` (`vi` if unset). Both create an empty one if there isn't any. The file holds the `[[projects]]` `-g` spans, `branch_prefix` — what `ccwt new` puts in front of a worktree's name to make its branch (`worktree-` when unset) — `columns`, which columns the table draws, `sort`, the order the worktrees come in (`freshness` when unset), `task_command`, the agent cli a queued prompt is run with and the tui's `c` starts (`claude` when unset), `forges`, which cli finds a host's reviews for the tui's `m`, and `environments`, what `ccwt mr`'s `ENV` column calls each GitLab environment (`production = "prod"`). |
