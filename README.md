...
git makes of the branch or the remote, and `±` means the same on a merged branch — the changes
are the only thing in there that isn't on `main` yet; `✳` beats them all and means an
...
`forges` says which cli knows about a host's reviews, for the tui's [`m`](#the-tui) key and
for [`ccwt mr`](#command-reference), which asks GitHub for a `"gh"` host and GitLab for any
other. Only a host whose name doesn't give it away needs a line — a self-hosted GitLab most
of all, since `github.com`, `gitlab.com` and anything with `gitlab` in the name are guessed:
...
`environments` renames an environment for [`ccwt mr`](#command-reference)'s `ENV`
column: the name GitLab or GitHub knows it by on the left, what to call it on the right.
Deployment pipelines name environments to be unambiguous in a settings page, which is
longer than a column you only glance at wants:
...
The lookup is a handful of GitLab or GitHub round trips, so it runs in the background and is
asked again at most once a minute: the table you're reading never waits on the network, and
the row appears under the heading a moment after the review does.
...
**A workspace whose `ccwt ws` has gone.** The tab `c` starts `ccwt ws` in is named `ws`, and
the name outlives the tui: quit it, or restart Herdr — which brings the tabs back but not what
was running in them — and the tab is a bare shell under the same name. The tui names each of
...
It listens to Herdr's events, and looks again whenever a tab or a workspace is made, renamed
or closed — and every ten seconds besides, since Herdr says nothing when a program exits back
to its shell. It covers only the workspaces open on the worktrees it lists — the ones Herdr
shows under the repo, or under any configured project with `-g` — and leaves another repo's to
...
| `ccwt gc` | Remove every worktree that's finished with: branch already merged (the `✓` of `ccwt list`), nothing uncommitted in it (no `*` or `±`), no agent working in it (no `✳`) and no agent session running in it (a `no` in the `AGENT` column). Prints the list it found and asks before touching anything — `-y`/`--yes` skips the question. Each removal is exactly what `ccwt remove <name>` does, branch included. The worktree you're standing in is never removed — it says so on stderr and leaves it to `ccwt remove .`. |
