...
was running in them — and the tab is a bare shell under the same name. The tui names every
workspace in that state, in a yellow box in its bottom-right corner:
...
to its shell. It covers all of Herdr's workspaces, not only the repo's. Only the tui does this,
not `ccwt ws`, and only when it is running in a Herdr pane.
